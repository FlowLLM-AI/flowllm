...
class EsVectorStore(LocalVectorStore):
    ...
    def utility_based_delete(self, workspace_id: str, freq_threshold: int, utility_threshold: float,
        ...
        if delete_node_ids:
            logger.info(
                f"Found {len(delete_node_ids)} nodes to delete with freq>={freq_threshold} and utility/freq<{utility_threshold}")
